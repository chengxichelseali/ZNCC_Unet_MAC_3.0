...
def add_coordinate_channels(tensor):
    """
    Keep your original input design:
    input = [image, x_channel, y_channel] => 3 channels for grayscale image.
    NOTE: CNN doesn't need requires_grad coords anymore.
    """
...
def _extract_uv(pred):
    """
    pred can be [B,5,H,W] or [B,2,H,W].
    Return u,v as [B,1,H,W].
    """
    if pred.shape[1] >= 2:
        u = pred[:, 0:1]
        v = pred[:, 1:2]
